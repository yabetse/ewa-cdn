...
var base_salary = parseFloat(
  $("#view_51 .field_44 .kn-detail-body").text().replace(/,/g, "") == ""
    ? 0
    : $("#view_51 .field_44 .kn-detail-body").text().replace(/,/g, "")
);
var requested_amount = parseFloat(
  $("#view_52 .kn-pivot-calc:eq(0)").text().replace(/,/g, "") == ""
    ? 0
    : $("#view_52 .kn-pivot-calc:eq(0)").text().replace(/,/g, "")
);
var requested_transactions = parseInt(
  $("#view_52 .kn-pivot-calc:eq(1)").text().replace(/,/g, "") == ""
    ? 0
    : $("#view_52 .kn-pivot-calc:eq(1)").text().replace(/,/g, "")
);

var max_number_requests = parseFloat(
  $("#view_68 .field_91 .kn-detail-body").text().replace(/,/g, "") == ""
    ? 0
    : $("#view_68 .field_91 .kn-detail-body").text().replace(/,/g, "")
);
var withdrawable_threshold = parseFloat(
  $("#view_68 .field_89 .kn-detail-body").text().replace(/,/g, "") == ""
    ? 0
    : $("#view_68 .field_89 .kn-detail-body").text().replace(/,/g, "")
);
...
var html =
  '<section id="custom-view-scene1">' +
  '<div class="payday-wrapper">' +
  "<div>" +
  '<div class="payday-label">Next Payday</div>' +
  '<div class="payday-value">' +
  payday +
  "</div>" +
  '<span class="cutoff-message"><i>Withdrawals until ' +
  cutoff_day +
  "</i></span>" +
  "</div>" +
  ...
  "<span>" +
  (available_amount * withdrawable_threshold)
    .toFixed(2)
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
  "</span>" +
  "<a" +
  (check == true
    ? ' href="https://dasint.knack.com/earned-wage-access-live#request"'
    : ' style="pointer-events:none;" class="disabled"') +
  ">Withdraw</a>" +
  "</div>" +
  "</div>" +
  "</section>";
